...
export function FloatingQuoteButton() {
  ...
  return (
    <button
      onClick={handleClick}
      className="fixed right-6 top-1/2 -translate-y-1/2 md:right-6 md:top-1/2 md:-translate-y-1/2 bottom-6 right-1/2 translate-x-1/2 md:translate-x-0 md:bottom-auto z-50 group md:hover:scale-105 transition-all duration-300"
      aria-label="即時詢價"
    >
      {/* Main button */}
      <div className="bg-gradient-to-br from-emerald-500 to-teal-600 text-white w-16 h-16 md:w-16 md:h-16 w-20 h-12 rounded-lg md:rounded-lg rounded-full shadow-xl md:hover:shadow-2xl transition-all duration-300 flex flex-col md:flex-col flex-row items-center justify-center gap-2 md:gap-0">
        {/* LINE icon */}
        <svg 
          width="20" 
          height="20" 
        ...
        {/* Text */}
        <span className="text-xs md:text-xs text-xs font-medium leading-tight">
          即時詢價
        </span>
      </div>
      
      {/* Pulse animation - only on desktop */}
      <div className="hidden md:block absolute inset-0 bg-gradient-to-br from-emerald-500 to-teal-600 rounded-lg animate-ping opacity-20 -z-10"></div>
      
      {/* Glow effect - only on desktop */}
      <div className="hidden md:block absolute inset-0 bg-gradient-to-br from-emerald-400 to-teal-500 rounded-lg blur-sm opacity-30 group-hover:opacity-50 transition-opacity duration-300 -z-10"></div>
    </button>
  );
}