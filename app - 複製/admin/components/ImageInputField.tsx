...
function LibraryGrid({
  ...
  const hasContent = images.length > 0 || (folders && folders.length > 0);
  ...
  return (
    <div className="space-y-2">
      {/* 麵包屑導航 */}
      {currentFolder && currentFolder !== 'uploads/' && onBack && (
        <div className="flex items-center gap-2 text-xs text-zinc-600">
          ...
          <span className="text-zinc-900 font-medium">
            {currentFolder.replace('uploads/', '').replace(/\/$/, '')}
          </span>
        </div>
      )}
      ...
      {folders && folders.length > 0 && (
        <div className="grid grid-cols-3 gap-2 sm:grid-cols-4 md:grid-cols-5 mb-3">
          {folders.map((folder) => (
            <button
              key={folder.prefix}
              type="button"
              onClick={() => onFolderClick?.(folder.prefix)}
              className="flex flex-col items-center gap-1 rounded-md border border-zinc-200 bg-zinc-50 p-2 hover:border-zinc-400 hover:bg-zinc-100 transition"
            >
              <div className="text-2xl">📁</div>
              <span className="line-clamp-1 w-full text-center text-[10px] text-zinc-700 font-medium">
                {folder.name}
              </span>
...
export function ImageInputField({
  ...
  async function loadLibrary(folder: string = 'uploads/') {
    setIsLibraryLoading(true);
    try {
      const res = await fetch(`/api/admin/images?prefix=${encodeURIComponent(folder)}`);
      if (!res.ok) throw new Error("failed to load images");
      const data = await res.json();
      
      // 解析資料夾和檔案
      const folderMap = new Map<string, Folder>();
      const fileList: ImageAssetItem[] = [];
        ...
        data.images.forEach((item: any) => {
          ...
          if (parts.length > 1) {
            ...
            fileList.push({
              id: item.id || key,
              url: item.url,
              label: item.label || key.split('/').pop(),
            });
          }
        });
      }
    ...
    <div className="space-y-2">
      ...
      {value && (
        <div className="flex items-start gap-3 rounded-lg border border-zinc-200 bg-zinc-50 p-2">
          <div className="aspect-square w-16 overflow-hidden rounded-md bg-zinc-100">
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img
              src={value}
              alt=""
      ...
      {images.length > 0 && (
        <div className="flex flex-wrap gap-2 rounded-lg border border-zinc-200 bg-zinc-50 p-2">
          {images.map((url) => (
            <div
              key={url}
              className="group relative aspect-square w-16 overflow-hidden rounded-md border border-zinc-200 bg-zinc-100"
            >
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img
                src={url}
