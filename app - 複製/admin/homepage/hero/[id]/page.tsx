// app/admin/homepage/hero/[id]/page.tsx
...
/** Server action：更新「指定 HERO id」的 payload */
async function updateHeroPayload(formData: FormData) {
  "use server";

  const idRaw = formData.get("heroId")?.toString();
  if (!idRaw) return;
  const id = Number(idRaw);
  if (!Number.isInteger(id)) return;

  const hero = await prisma.homeSection.findUnique({
    where: { id },
  });
  ...
  return (
    <main className="mx-auto max-w-3xl px-4 py-10 md:px-6">
      ...
      <form action={updateHeroPayload} className="mt-6 space-y-6">
        <input type="hidden" name="heroId" value={hero.id.toString()} />
        ...
        <section className="space-y-4 rounded-md bg-zinc-50 p-4">
          ...
          <div className="space-y-2">
            ...
            <p className="text-[11px] text-zinc-500">
              目前先填圖片 URL（例如 /cdn/hero/hero-giftbox.jpg），未來再加上傳功能。
            </p>
          </div>
        </section>
